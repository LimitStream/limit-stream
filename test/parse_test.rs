...
gen_test!(_type, type_test, "recv string -> send int -> send int -> end", Type::Session(SessionType(vec![
  Session::Recv(TypeOrName::Type(Box::new(Type::SimpleType(SimpleType::String)))),
...
gen_test!(struct_def, struct_def_test, "
struct User {
  name: string = 0,
  age: uint = 1,
  desc: string = 2
...
", StructDef {
  ...
  records: vec![
    ("name", (TypeOrName::Type(Box::new(Type::SimpleType(SimpleType::String))), Some(0))),
    ("age", (TypeOrName::Type(Box::new(Type::SimpleType(SimpleType::Uint))), Some(1))),
    ("desc", (TypeOrName::Type(Box::new(Type::SimpleType(SimpleType::String))), Some(2))),
    ] });

gen_test!(enum_item, enum_item_test, "Admin(user) = 0
", ("Admin", (TypeOrName::Name("user"), Some(0))));

gen_test!(enum_def, enum_def_test, "
enum usertype {
  Admin(user) = 0,
  Normal(user) = 1,
  Visitor(visitor) = 2
